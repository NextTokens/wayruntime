...
/*
 * cpu_features.c (POSIX) — one-shot CPU feature probe for the SIMD
 * dispatcher.
 *
 ...
 * host cannot run.
 *
 * On x86 the probe is __builtin_cpu_supports, which (via libgcc's
 * feature init) also honors OSXSAVE/XCR0, so "supported" means usable
 * from userland — no XCR0 poking here, the OS owns extended state.
 * On Linux/arm64 it is getauxval(AT_HWCAP) & HWCAP_ASIMD.  Anything
 * else probes to 0 and the dispatcher stays scalar.
 */
...
/* riscv64 and friends: scalar-only by design. */
static uint32_t wri_probe_features(void)
{
    return 0;
}
